...
app.use(expressJWT({
	...
	path: ['/users', '/createAfflication']
}));
app.use(bearerToken());
app.use(function (req, res, next) {
	logger.debug(' ------>>>>>> new request for %s', req.originalUrl);
	if (req.originalUrl.indexOf('/users') >= 0 || req.originalUrl.indexOf('/createAfflication') >= 0) {
		return next();
	}
...
// Create affliation for new organization
app.post('/createAffliation', async function (req, res) {
	try {
		var orgnumber = req.body.orgnumber;
		var port = req.body.port;
		var copService = require('fabric-ca-client');
		var client = await helper.getClientForOrg(`Org${orgnumber}`)
		var affs = new copService(`https://localhost:${port}`);
		var admins = hfc.getConfigSetting('admins');
		let adminUserObj = await client.setUserContext({
			username: admins[0].username,
			password: admins[0].secret
		});
		affs.newAffiliationService().create({
			name: `org${orgnumber}`
		}, adminUserObj)
		affs.newAffiliationService().create({
			name: `org${orgnumber}.department1`
		}, adminUserObj)
		res.send({
			"status": "success"
		})
